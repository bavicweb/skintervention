...
class BridgeCoreImport {
	...
	public function import_settings_pages( $file ) {
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'bridge-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'bridge-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'bridge-core'));
		}
	}
	...
	function import_get_demo_urls( $folder ) {
		$demo_urls  = array();
		$domain_url = defined( 'MASTERDS_CORE_PROFILE_SLUG' ) ? str_replace( '/', '', $folder ) . '.' . MASTERDS_CORE_PROFILE_SLUG . '-themes.com/' : '';
